...
import { Table_status, Tables } from '../table/table_entity/table.entity'
import { Staff } from 'src/staff/staff.entity/staff.entity';
import { OrderItems, STATUS_ORDERITEMS } from '../order_items/order_items.entity'
...
export class OrderService {
    ...
    async addOrder(dto: CreateOrderDto, user) {

        const table = await this.tableEntity.findOne({ where: { id: Number(dto.table_id) }, relations: ['order'] })
        console.log(table)
        if (!table) { throw new NotFoundException("table not found") }

        if (table.status === Table_status.Opened && !table.order) {
            //         //Заполняем rder
            console.log('33333333')
            const worker = await this.staffEntity.findOne({ where: { id: Number(user) } })
            ...
            await this.tableEntity.update({ id: table.id }, { status: Table_status.Cloased, order: saved_order, chacking_status: Chacking_status.NotChacking })

            return { saved_order, order_itemsArray }


        } else if (table.status === Table_status.Cloased && table.order) {
            ...
            for (let [foodId, quantity] of order_item_map) {
                ...
                if (ex) {
                    ...
                    ex.status = STATUS_ORDERITEMS.NOT_STARTED
                    await this.orderItemsEntity.save(ex)
                } else {
                    const new_or_item = await this.orderItemsEntity.create({ order: order, food: food, quantity: quantity, status: STATUS_ORDERITEMS.NOT_STARTED })
    ...
    async deleteFoodInOrder(id: string, dto: DeleteFoodInOrderDto) {
        ...
        const order = await this.orderEntity.findOne({ where: { table: { id: Number(id) } }, relations: ['orderItems', 'orderItems.food'] })
        if (!order) { throw new NotFoundException('order not found') }
        ...
        return await { order: this.orderEntity.save(order), order_items: await this.orderItemsEntity.findBy({ order: { id: order.id } }) }
        //const orderItemsByOrder = await this. 

    }

    async all_orders(dto: FilterOrderDto) {
        const where: any = {}

        if (dto.tableId) { where.table = dto.tableId }
        if(dto.workerId){where.worker = dto.workerId}


        if(dto.minSumm && dto.maxSumm){
            where.total_summ = Between(dto.minSumm, dto.maxSumm)
        }else if(dto.minSumm && !dto.maxSumm){
            where.total_summ = MoreThanOrEqual(dto.minSumm)
        }else if(!dto.minSumm && dto.maxSumm){
            where.total_summ = LessThanOrEqual(dto.maxSumm)
        }

        if (dto.createdFrom && dto.createdTo) {
            const dateFrom = new Date(dto.createdFrom)
            const dateTo = new Date(dto.createdTo)
            ...
            where.created = MoreThanOrEqual(dateFrom)
        } else if (!dto.createdFrom && dto.createdTo) {
            const dateTo = new Date(dto.createdTo)
            dateTo.setHours(23, 59, 59, 999)

            where.created = LessThanOrEqual(dateTo)
        }
    ...
    async delete_order(id){
        const order = await this.orderEntity.findOne({where: {id: Number(id)}, relations:['table', 'orderItems']})
        if(!order){throw new NotFoundException('order not found')}
        
        const order_items = await this.orderItemsEntity.find({where: {order: {id: order.id}}})
        await this.orderItemsEntity.delete({ order: { id: order.id } });


        const table = await this.tableEntity.findOne({where:{order: {id: order.id}}})
        ...
        table.status = Table_status.Opened
        await this.tableEntity.save(table)

        return {message: 'order deleted. table is opened'}
    }
    ...
    //         if (!table || table.status === Table_status. || table.status === 2) { throw new NotFoundException('table not found or invalid table_id') }
